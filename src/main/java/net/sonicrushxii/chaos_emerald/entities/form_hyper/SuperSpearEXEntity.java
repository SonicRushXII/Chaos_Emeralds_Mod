...
public class SuperSpearEXEntity extends LinearMovingEntity {
    public static final EntityDataAccessor<Boolean> DESTROY_BLOCKS = SynchedEntityData.defineId(SuperSpearEXEntity.class, EntityDataSerializers.BOOLEAN);
    public static final EntityDataAccessor<Optional<UUID>> OWNER = SynchedEntityData.defineId(SuperSpearEXEntity.class, EntityDataSerializers.OPTIONAL_UUID);
    private int MAX_DURATION = 200;
    private static final float STRENGTH = 1.85F;
    private static final float DAMAGE = 8.0F;

    public SuperSpearEXEntity(EntityType<? extends PointEntity> type, Level world) {
        super(type, world);
    }
    ...
    public void tick() {
        ...
        if(this.level().isClientSide) {
            Utilities.displayParticle(this.level(), new DustParticleOptions(new Vector3f(0.9f, 1f, 1f), 1f),
            ...
            List<Entity> enemies = this.level().getEntitiesOfClass(Entity.class,
                    ...
                    enemy -> !(enemy instanceof SuperSpearEXEntity)
            );
